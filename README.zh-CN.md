...
- 边写边校：第一处错误直接标在正文上，状态栏显示有几处问题，点一下跳过去并说清错在哪
...
- 不做 Markdown 预览：这里是纯文本编辑器
- 着色是按行猜的，不是编译器：嵌套块注释、Rust 的 `r#"…"#`、Ruby / Shell 的 heredoc
  都不解析，跨行结构只认「一个开始符 + 一个结束符」
- 不带 BOM 的 UTF-16 会被当成二进制（先用别的工具转成 UTF-8 就能打开）；前 8000 个字符
  之后的 NUL 检测不到
- 校验是本地的、单文件的（目前只有 JSON）：没有补全、悬停、跳转定义，也没有工程级分析
- 不捆绑字体，只用系统自带的三种；列号按 UTF-16 算，一个 emoji 占两列
- 软换行只对可编辑的文档生效：只读浏览按行加载，折行要重算每一行，所以它不折
- 不单独识别 Big5：GB18030 已经覆盖它的编码空间
